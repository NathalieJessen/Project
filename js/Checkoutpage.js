...
function validator(){
    ...
    if(!checkInput(expmonth, "^[0-9]{2}$" )){
        alert('You have entered an invalid email month')
        return false;
    }

    if(!checkInput(expyear, "^[0-9]{4}$" )){
        alert('You have entered an invalid email year')
        return false;
    }
...
document.getElementById("confirm").addEventListener("click", function(event) {
    ...
    if(validator()){
        ...
        var creditCards = []
            ...
            creditCards.push(new CreditCard (cname,ccnum,expmonth, expyear, cvv));
